...
template<typename T>
vector<vector<T>> permutate(vector<T> &array, int r) {
	if (r == -1)
		r = array.size();
	vector<vector<T> > result;
	...
	for (vectorIt = array.begin(); vectorIt != array.end(); ++vectorIt) {
		...
		vector<vector<T> > results = permutate(copy, r - 1);
...
void print_vector(const vector<char>& array, std::ostream& out) {
	out << "[";
	for (int i = 0; i < array.size(); i++) {
		out << array[i];
		if (i != array.size() - 1)
...
template vector<vector<int> > permutate(vector<int>&, int r);
